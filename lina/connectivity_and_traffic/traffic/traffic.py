...
def traffic_calc(output):
    ...
    total_input_1m, total_input_pps_1m = sum(v[0] for v in input_rates_1m.values()), sum(v[1] for v in input_rates_1m.values())
    total_input_5m, total_input_pps_5m = sum(v[0] for v in input_rates_5m.values()), sum(v[1] for v in input_rates_5m.values())
    total_output_1m, total_output_pps_1m = sum(v[0] for v in output_rates_1m.values()), sum(v[1] for v in output_rates_1m.values())
    total_output_5m, total_output_pps_5m = sum(v[0] for v in output_rates_5m.values()), sum(v[1] for v in output_rates_5m.values())

    # Print tables
    print("\nINPUT TRAFFIC RATES:")
    traffic_table(["Interface", "1-Min Input (Bps)", "1-Min Input (pps)", "5-Min Input (Bps)", "5-Min Input (pps)"], input_rates_1m, input_rates_5m)

    print("\nOUTPUT TRAFFIC RATES:")
    traffic_table(["Interface", "1-Min Output (Bps)", "1-Min Output (pps)", "5-Min Output (Bps)", "5-Min Output (pps)"], output_rates_1m, output_rates_5m)
