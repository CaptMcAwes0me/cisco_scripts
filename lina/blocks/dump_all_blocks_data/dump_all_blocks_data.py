...
def dump_all_blocks_data():
    ...
    log_dir = os.path.join(troubleshooting_dir, f"blocks_dump_{timestamp}")

    try:
        os.makedirs(log_dir)  # Create a subdirectory for this dump
    except Exception as e:
    ...
    commands = {
        "blocks.log": blocks,
        "blocks_exhaustion_history.log": blocks_exhaustion_history,
        "blocks_exhaustion_snapshot.log": blocks_exhaustion_snapshot,
        "blocks_queue_history_core_local.log": blocks_queue_history_core_local,
        "blocks_queue_history_detail.log": blocks_queue_history_detail,
        "blocks_old.log": blocks_old,
        "blocks_old_dump.log": blocks_old_dump
    }

    # Execute each command and write to separate log files
    ...
    # Compress all logs into a .tar.gz archive
    archive_path = os.path.join(troubleshooting_dir, f"blocks_dump_{timestamp}.tar.gz")
    try:
        with tarfile.open(archive_path, "w:gz") as tar:
            tar.add(log_dir, arcname=os.path.basename(log_dir))
        print(f"[+] Compressed logs into: {archive_path}")

        # Cleanup: Remove the temporary log directory
