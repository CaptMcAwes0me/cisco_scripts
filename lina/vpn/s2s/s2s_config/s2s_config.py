...
def s2s_policy_based_config(ip_address, ike_version):
    ...
    policy_match = re.search(rf"crypto map (\S+) (\d+) .* set peer {re.escape(ip_address)}", policy_output)

    if policy_match:
        crypto_map_name = policy_match.group(1)
        crypto_map_number = policy_match.group(2)
        crypto_map_output = get_and_parse_cli_output(f"show running-config crypto map | include {crypto_map_name} {crypto_map_number}")
